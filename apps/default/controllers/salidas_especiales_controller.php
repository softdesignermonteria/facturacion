<?php
	...
	class salidas_especialesController extends ApplicationController {
		...
		public function initialize() {
			...
			$this->SalidasEspeciales->setLogger("SalidasEspecialesDebug.txt");
			$this->Kardex->setLogger("KardexDebug.txt");
			$this->DetalleSalidasEspeciales->setLogger("DetalleSalidasEspecialesDebug.txt");
			$this->DetalleSalidasEspecialesTmp->setLogger("DetalleSalidasEspecialesTmpDebug.txt");
	 ...
	 public function beforeFilter(){
			...
			if($sw==0){
				...
				try{
					...
					foreach( $detalles_item as $items):
						$detalles = new DetalleSalidasEspeciales();
						...
						$detalles->salidas_especiales_id  = $ent->id;
						...
						$total +=  $detalles->total;
							if($detalles->save()==false){
								foreach($detalles->getMessages() as $message){ 
									Flash::error(" TABLA DETALLE SALIDAS ESPECIALES : ".$message); 
								}
								$transaction->rollback();
							}	
					endforeach;			
				...
				$transaction->commit();
				Flash::success("SALIDAS ESPECIAL GUARDADA SATISFACTORIAMENTE");	
				echo "<script>alert('SALIDAS ESPECIAL GUARDADA SATISFACTORIAMENTE');";
				echo "redireccionar_action('salidas_especiales/agregar/?id=$ent->id&=msgconfirm=true');</script>";
		...
		public function showAction($id){
			 ...
			 $this->setParamToView("encabezado",$this->SalidasEspeciales->find(" id = '$id' "));
			 $this->setParamToView("idt",$id);								
			 $this->setParamToView("detalles",$this->DetalleSalidasEspeciales->find(" salidas_especiales_id = '$id' and anulado = 0"));				
		...
		public function detalle_buscarAction(){
				...
				if( isset( $_REQUEST['fecha_inicio']       )==true ) { if( $_REQUEST['fecha_inicio'] != ''       ) { $sql .= " and {#SalidasEspeciales}.fecha >=  '".$_REQUEST['fecha_inicio']."'";           } }
				if( isset( $_REQUEST['ffinal']         )==true ) { if( $_REQUEST['ffinal'] != ''         ) { $sql .= " and {#SalidasEspeciales}.fecha <=  '".$_REQUEST['ffinal']."'";           } }
				
				$empresa_id  = $_REQUEST["empresa_id"];
				$empresa_id  = Session::get("id_empresa");
			
				$query = new ActiveRecordJoin(array(
						"entities" => array("SalidasEspeciales","Proveedores","Empresa","TipoDocumento"),
					 	"fields"=>array(
										"{#SalidasEspeciales}.id",
										"{#SalidasEspeciales}.prefijo",
										"{#SalidasEspeciales}.consecutivo",
										"{#SalidasEspeciales}.fecha",
										"{#Proveedores}.razon_social"
										),
						"conditions"=>(" {#SalidasEspeciales}.empresa_id = '$empresa_id' $sql ")
				));
